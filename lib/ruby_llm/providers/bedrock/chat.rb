  ...
  module Providers
    class Bedrock
      ...
      module Chat
        ...
        def render_payload(messages, tools:, temperature:, model:, stream: false, schema: nil, thinking:, thinking_budget:) # rubocop:disable Lint/UnusedMethodArgument,Metrics/ParameterLists
          @model_id = model.id
          ...
          build_base_payload(chat_messages, model).tap do |payload|
            Anthropic::Chat.add_optional_fields(payload, system_content:, tools:,  temperature:, thinking:, thinking_budget:)
          end
        end
