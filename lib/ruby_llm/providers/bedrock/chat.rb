  ...
  module Providers
    class Bedrock
      ...
      module Chat
        ...
        def render_payload(messages, tools:, temperature:, model:, # rubocop:disable Metrics/ParameterLists
                           thinking:, thinking_budget:, stream: false, schema: nil) # rubocop:disable Lint/UnusedMethodArgument
          @model_id = model.id
          ...
          build_base_payload(chat_messages, model).tap do |payload|
            Anthropic::Chat.add_optional_fields(payload, system_content:, tools:, temperature:, thinking:,
                                                         thinking_budget:)
          end
        end
