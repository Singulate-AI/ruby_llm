      ...
      module Images
        ...
        def parse_image_response(response, model:)
          ...
          image_data = if uses_generate_content?(model)
                         ...
                         { 'bytesBase64Encoded' => raw_data&.fetch('data', nil),
                           'mimeType' => raw_data&.fetch('mimeType', nil) }
                       else
                         data['predictions']&.first
                       end

          unless image_data&.key?('bytesBase64Encoded') && image_data['bytesBase64Encoded']
        ...
        # Gemini models that use the generateContent endpoint for image generation
        # rather than the predict endpoint. Checked when metadata lookup fails.
        GENERATE_CONTENT_IMAGE_MODELS = /gemini.*image|flash-image/i
        ...
        def uses_generate_content?(model_id)
          @generate_content_cache ||= {}
          return @generate_content_cache[model_id] if @generate_content_cache.key?(model_id)

          @generate_content_cache[model_id] = check_generate_content(model_id)
        end

        def check_generate_content(model_id)
          model = RubyLLM::Models.find(model_id, @provider.slug)
          supported_methods = model.metadata[:supported_generation_methods]
          return supported_methods.include?('generateContent') if supported_methods

          # Fall back to model name pattern when metadata is missing
          model_id.to_s.match?(GENERATE_CONTENT_IMAGE_MODELS)
        rescue ModelNotFoundError
          # Model not in registry — infer from model name
          model_id.to_s.match?(GENERATE_CONTENT_IMAGE_MODELS)
        end
      end
    end
