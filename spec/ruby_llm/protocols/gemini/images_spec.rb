...
RSpec.describe RubyLLM::Protocols::Gemini::Images do
  let(:provider_double) { instance_double(RubyLLM::Providers::Gemini, slug: 'gemini') }
  ...
  let(:test_instance) do
    klass = Class.new do
      ...
      # Provide a @provider whose slug is used by check_generate_content
      def initialize(provider)
        @provider = provider
      end
    end
    klass.new(provider_double)
  end
